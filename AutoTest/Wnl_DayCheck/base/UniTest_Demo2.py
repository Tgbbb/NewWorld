...
class TestMethod(unittest.TestCase):
    ...
    def test1(self):  # unitest里面case定义必须为test开头，且是按定义的数字顺序来执行的
    ...
    def test2(self):
        ...
        print('这个是第二条测试case')
        res = self.runner.run_main('POST', url, headers, data)
        self.assertEqual(res['status'], 100, '第二条case验证失败！○･｀Д´･ ○')  # ！！！unitest的断言方法，assert，还有几种自行查找
...
if __name__ == '__main__':
    ...
    filepath = "d:/report/htmlreport.html"  # 创建报告存储路径
    fp = open(filepath, 'wb')
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title='01Testreport')
    runner.run(suite)
